...
class RXArm(InterbotixRobot):
    ...
    def __init__(self, dh_config_file=None):
        ...
        self.effort_fb = None
        # DH Params
        self.SDxA=self.position_fb
        #print(f"pos:- {self.SDxA}")
        
        #0.17415
        self.dh_params = np.array([[np.pi/2.0,              0.10391,    0.00,       np.pi/2.0],
                                   [-103.56*(np.pi/180.0),   0.00,       -0.2057,    0.00],
                                   [103.56*(np.pi/180.0),    0.00,       0.2 ,       0.00],
                                   [np.pi/2.0,              0.00,       0.00,       np.pi/2.0],
                                   [   0,                   0.17415,    0.00,       0.00]])
        
        self.dh_config_file = None #dh_config_file
    ...
    def get_IK_joint_pose(self, world_coords):
...
#   @_ensure_initialized
    ...
    def get_ee_pose(self):
        ...
        """
        #print(self.dh_params)
        self.joint_angle= list(self.get_joint_positions())
        
        Temp=FK_dh(self.dh_params,self.joint_angle)
        #self.pos = get_pose_from_T(Temp)
        #rospy.sleep(5)
        #return self.pos
        return [Temp[0,3], Temp[1,3], Temp[2,3], 0,0,0]
